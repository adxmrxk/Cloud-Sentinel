...
from tests.helpers import AUDITOR_PAYLOAD, AUTH, README_PAYLOAD, post
...
@pytest.mark.parametrize(
    "body, status", [("not json", 400), ("", 400), ("[1, 2, 3]", 422)]
)
def test_non_object_body_is_rejected(client, body, status):
...
def test_body_without_content_type_is_accepted(client):
    response = client.post("/ingest", data=json.dumps(README_PAYLOAD), headers=AUTH)
    assert response.status_code == 201
...
def test_storage_failure_returns_500(client, monkeypatch):
    ...
    assert "dynamodb down" not in response.get_json()["message"]
