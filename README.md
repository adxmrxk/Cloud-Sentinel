...
```
   ...
   1.  Auditor Lambda  (.NET 8)
       ...
       • Returns JSON: at-risk buckets + severity scores
                   │
                   ▼
   2.  Reporter Lambda  (Python / Flask)   every audit, clean ones included
       • POST /ingest with audit results
       • Writes findings to DynamoDB
       • Records audit metadata + timestamp
                   │
                   ▼
   3.  Choice State: any vulnerabilities found?
       │
   ...
   4.  SNS Topic: security alert published
       └── Subscribe Slack, email or PagerDuty to the topic
                   │
                   ▼
        ┌──────────────────────┐
...
- **Every audit is recorded; only findings alert.** Clean scans still reach the reporter, so the dashboard and metrics show that a scan ran. The Choice state sends the SNS alert only when vulnerabilities are found.
